...
#define PH_CRYPTORNG_MBEDTLS_ID                                         0x03U   /**< ID for mBedTLS crypto rng component. */

/** \defgroup phCryptoRng_mBedTLS Component : mBedTLS
 * \brief mBedTLS implementation of the Random Number interface
 * @{
 */

 /** \brief Data structure for Random Number's mBedTLS layer implementation. */
typedef struct
{
    uint16_t wId;                                                               /**< Layer ID for this component, NEVER MODIFY! */
    void * pCtx_Drbg;                                                           /**< Pointer to underlying crypto library's random number context. */
    void * pCtx_Entropy;                                                        /**< Pointer to underlying crypto library's random number entropy context. */
    int32_t dwErrorCode;                                                        /**< Error code returned by mbedTLS layer. */
} phCryptoRng_mBedTLS_DataParams_t;

/**
 * \brief Initialise the CryptoRng component.
 *
 * \return Status code
 * \retval #PH_ERR_SUCCESS                  Operation successful.
 * \retval #PH_ERR_INVALID_DATA_PARAMS
 ...
 *          - If the DataParams are null.
 */
phStatus_t phCryptoRng_mBedTLS_Init(
        phCryptoRng_mBedTLS_DataParams_t * pDataParams,                         /**< [In] Pointer to this layers parameter structure. */
        uint16_t wSizeOfDataParams                                              /**< [In] Specifies the size of the data parameter structure. */
    );

/**
 * \brief De-Initialize the CryptoRng with mBedTLS as sub-component.
 ...
 */
phStatus_t phCryptoRng_mBedTLS_DeInit(
        phCryptoRng_mBedTLS_DataParams_t * pDataParams                          /**< [In] Pointer to this layer's parameter structure. */
    );

/**
 * end of group phCryptoRng_mBedTLS
 * @}
 */
