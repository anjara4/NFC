...
void phhalVca_Sw_Int_BackupIV(phalVca_Sw_DataParams_t * pDataParams, uint8_t *pIv_Bak)
{
    ...
    switch(PH_GET_COMPID(pDataParams->pCryptoDataParams))
    {
        ...
        case PH_CRYPTOSYM_MBEDTLS_ID:
            pIV_Tmp = ((phCryptoSym_mBedTLS_DataParams_t *) pDataParams->pCryptoDataParams)->aIV;
            if(pIV_Tmp != NULL)
            {
                (void) memcpy(pIv_Bak, pIV_Tmp, PH_CRYPTOSYM_AES128_KEY_SIZE);
            }
...
#ifdef NXPBUILD__PH_CRYPTOSYM_MBEDTLS
        case PH_CRYPTOSYM_MBEDTLS_ID:
            pKey_Tmp = ((phCryptoSym_mBedTLS_DataParams_t *) pDataParams->pCryptoDataParams)->aKey;
            if(pKey_Tmp != NULL)
            {
                (void) memcpy(pKey_Bak, pKey_Tmp, PH_CRYPTOSYM_MAX_BLOCK_SIZE);
            }
